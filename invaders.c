...
int	main(int argc, char **argv)
{
	...
	for (;;)
	{
		...
		case ' ':
			...
			else if (game.state == STATE_PLAY && game.timer > GUNNER_ENTRANCE
				&& gun.missile.y == 0)
			{
				gun.missile.x = gun.x;
...
void	handleTimer(int signal)
{
	...
	else if (game.state == STATE_EXPLODE)
	{
		...
		if (gun.explodeTimer == 40)
		{
			...
			else
			{
				...
				lastLine = aliens.y + (ALIEN_HEIGHT * aliens.emptyBottom) - 1;
				shieldTop = LINES - GUNNER_HEIGHT - SHELTER_HEIGHT - 1;
	...
	if (ma.x > 0)
	{
		...
		else
		{
			...
			for (i = 0; i < MA_HEIGHT; i++)
				mvprintw(2 + i, ma.x, "%s ", sprites[MA].frames[0][i]);
			refresh();
		...
		for (x = aliens.emptyLeft; x < aliens.emptyRight; x++)
		{
			...
			for (y = aliens.emptyBottom - 1; y >= 0; y--)
			{
				if (aliens.table[(aliens.cols * y) + x] > ALIEN_EMPTY)
				{
					...
					{
						addBomb(aliens.x + (x * ALIEN_WIDTH) + (ALIEN_WIDTH
								/ 2), aliens.y + (y * ALIEN_HEIGHT)
							+ ALIEN_HEIGHT - 1);
					}
					break ;
				}
	...
	for (b = aliens.headBomb; b != NULL;)
	{
		...
		if (++(b->y) < LINES)
		{
			if (gun.missile.y != 0 && abs(b->x - gun.missile.x) < 2 && abs(b->y
					- gun.missile.y) < 2)
			{
				...
				&& b->y >= LINES - GUNNER_HEIGHT && b->x > (gun.x
					- (GUNNER_WIDTH / 2)) && b->x < (gun.x + (GUNNER_WIDTH
						/ 2)))
			{
			...
			else if (b->y < LINES - 1 - GUNNER_HEIGHT && b->y >= LINES - 1
				- GUNNER_HEIGHT - SHELTER_HEIGHT && gun.shields[((b->y - (LINES
							- 1 - GUNNER_HEIGHT - SHELTER_HEIGHT))
					* game.screenCols) + b->x] != ' ')
			{
				// collision with shield
				gun.shields[((b->y - (LINES - 1 - GUNNER_HEIGHT
							- SHELTER_HEIGHT)) * game.screenCols) + b->x] = ' ';
				mvaddch(b->y, b->x, ' ');
				removeBomb(b);
			}
			else
	...
	if (gun.missile.y != 0)
	{
		...
		if (gun.missile.y < LINES - 1 - GUNNER_HEIGHT && gun.missile.y >= LINES
			- 1 - GUNNER_HEIGHT - SHELTER_HEIGHT)
		{
			if (gun.shields[((gun.missile.y - (LINES - 1 - GUNNER_HEIGHT
							- SHELTER_HEIGHT)) * game.screenCols)
				+ gun.missile.x] != ' ')
			{
		...
		// test for collision with aliens
			...
			+ (ALIEN_WIDTH * aliens.cols) && gun.missile.y < aliens.y
			+ ALIEN_HEIGHT * aliens.rows && gun.missile.y >= aliens.y)
		{
			x = gun.missile.x - aliens.x;
			y = gun.missile.y - aliens.y;
		...
		else if (ma.x != 0 && gun.missile.y <= 2 + MA_HEIGHT
			&& gun.missile.x >= ma.x && gun.missile.x <= ma.x + ALIEN_WIDTH)
		{
			// chose a 'random' number of points, either 50, 100 or 150
			points = ((time(0) % 3) + 1) * 50;
			game.score += points;
			// remove ma
			for (i = 0; i < MA_HEIGHT; i++)
				mvprintw(2 + i, ma.x, "%s ", alienBlank);
...
void	moveAliensDown(void)
{
	...
	lastLine = ++aliens.y + (ALIEN_HEIGHT * aliens.emptyBottom) - 1;
	topShield = LINES - GUNNER_HEIGHT - SHELTER_HEIGHT - 1;
	gunnerTop = LINES - GUNNER_HEIGHT - 1;
	if (lastLine >= topShield && lastLine < topShield + SHELTER_HEIGHT)
	{
		// clear the shield line
		i = (lastLine - topShield) * game.screenCols;
...
void	trimAliens(void)
{
	...
	for (;;)
	{
		...
		{
			if (aliens.table[(i * aliens.cols)
				+ aliens.emptyLeft] != ALIEN_EMPTY)
			{
				found = 1;
				break ;
			}
	...
	for (;;)
	{
		for (i = 0; i < aliens.rows; i++)
		{
			if (aliens.table[(i * aliens.cols) + aliens.emptyRight
				- 1] != ALIEN_EMPTY)
			{
				found = 1;
				break ;
	...
	for (;;)
	{
		...
		{
			if (aliens.table[((aliens.emptyBottom - 1) * aliens.cols)
				+ i] != ALIEN_EMPTY)
			{
				found = 1;
				break ;
...
void	paintGunner(void)
{
	...
	for (i = 0; i < GUNNER_HEIGHT; i++)
	{
		...
		{
			printw("%s", sprites[GUNNER_EXPLODE].frames[(game.timer / 4) % 2][i]);
		}
	}
}
...
void	paintExplodingAlien(int y, int x)
{
	...
	for (i = 0; i < ALIEN_HEIGHT; i++)
	{
		...
		printw("%s", sprites[ALIEN_EXPLODE].frames[0][i]);
...
void	paintAlienRow(int row, int clean)
{
	...
	if (aliens.x > 0)
	{
		for (i = 0; i < ALIEN_HEIGHT; i++)
		{
			move((row * ALIEN_HEIGHT) + aliens.y + i, (aliens.emptyLeft
					* ALIEN_WIDTH) + aliens.x - 1);
			addch(' ');
		}
	}
	if (aliens.x + (ALIEN_WIDTH * aliens.emptyRight) < game.screenCols)
	{
		for (i = 0; i < ALIEN_HEIGHT; i++)
		{
			move((row * ALIEN_HEIGHT) + aliens.y + i, (aliens.emptyRight
					* ALIEN_WIDTH) + aliens.x);
			addch(' ');
		}
	}
	// draw the aliens
	for (x = aliens.emptyLeft; x < aliens.emptyRight; x++)
	{
		alien = aliens.table[(row * aliens.cols) + x];
		for (i = 0; i < ALIEN_HEIGHT; i++)
		{
			move((row * ALIEN_HEIGHT) + aliens.y + i, (x * ALIEN_WIDTH)
				+ aliens.x);
			switch (alien)
			{
			case ALIEN10:
			case ALIEN20:
			case ALIEN30:
				printw("%s", sprites[alien].frames[aliens.anim][i]);
				break ;
			case ALIEN_EXPLODE1:
...
void	paintIntro(void)
{
	...
	if (has_colors())
		attron(COLOR_PAIR(4));
#endif
	mvprintw(2, (COLS / 2) - 30, "    .___                                                     .___            ");
    mvprintw(3, (COLS / 2) - 30, "  __| _/____     ____   ____   ______________  _______     __| _/___________ ");
    mvprintw(4, (COLS / 2) - 30, " / __ |\\__  \\   / ___\\ / ___\\_/ __ \\_  __ \\  \\/ /\\__  \\   / __ |/ __ \\_  __ \\");
    mvprintw(5, (COLS / 2) - 30, "/ /_/ | / __ \\_/ /_/  > /_/  >  ___/|  | \\/\\   /  / __ \\_/ /_/ \\  ___/|  | \\/");
    mvprintw(6, (COLS / 2) - 30, "\\____ |(____  /\\___  /\\___  / \\___  >__|    \\_/  (____  /\\____ |\\___  >__|   ");
    mvprintw(7, (COLS / 2) - 30, "     \\/     \\///_____/_____/      \\/                  \\/      \\/    \\/       ");
#ifdef USE_COLORS
	if (has_colors())
		attron(COLOR_PAIR(2));
#endif
	mvprintw(9, (COLS / 2) - 9, sprites[MA].frames[0][0]);
	mvprintw(10, (COLS / 2) - 9, sprites[MA].frames[0][1]);
#ifdef USE_COLORS
	if (has_colors())
		attron(COLOR_PAIR(4));
#endif
	...
	mvprintw(13, (COLS / 2) - 8, sprites[ALIEN30].frames[0][1]);
	mvprintw(12, (COLS / 2), "= 30 points");
	mvprintw(15, (COLS / 2) - 8, sprites[ALIEN20].frames[0][0]);
	mvprintw(16, (COLS / 2) - 8, sprites[ALIEN20].frames[0][1]);
	mvprintw(15, (COLS / 2), "= 20 points");
	mvprintw(18, (COLS / 2) - 8, sprites[ALIEN10].frames[0][0]);
	mvprintw(19, (COLS / 2) - 8, sprites[ALIEN10].frames[0][1]);
	mvprintw(18, (COLS / 2), "= 10 points");
#ifdef USE_COLORS
	if (has_colors())
		attron(COLOR_PAIR(1));
#endif
	mvprintw(LINES - 1, (COLS - 41) / 2,
		"https://github.com/macdice/ascii-invaders");
	refresh();
}
...
void	resetShields(void)
{
	...
	for (x = 0; x < game.screenCols - 10; x += 10)
	{
		...
		{
			i = 0;
			sprite = &sprites[SHELTER].frames[0];
			while (sprite->frames[y][i] != 0)
			{
				gun.shields[(y * game.screenCols) + x
					+ i] = sprite->frames[y][i];
				i++;
			}
		}
...
int	removeBomb(struct Bomb *b)
{
	...
	for (this = aliens.headBomb, last = NULL; this != NULL; last = this,
		this = this->next)
	{
		if (this == b)
		{
